...
object VerifyTest extends TestSuite {
  test("cannot declare duplicate procedure parameters") {
    try {
      Verify.checkProc("my-proc", Proc(List(Formal(Id("x"), Sort("Int")), Formal(Id("x"), Sort("Int"))), List(), True, True, Block(List())))
  ...
  for (
    ...
    test("verify " + file) {
      ...
      val source: Source[ExtCmd] = runUnwrappingErrors(Verify(Source.file(in, ExtScript).cmds))
